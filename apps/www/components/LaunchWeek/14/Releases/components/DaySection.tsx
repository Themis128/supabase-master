...
// import { ArrowRightIcon } from '@heroicons/react/outline' // Disabled: missing module/type
        ...
        {day.shipped && day.steps.length > 0 ? (
          ...
          >
            <div className="relative text-foreground-light p-4 sm:px-6 md:py-6 md:px-8 z-20 flex-grow flex flex-col items-start justify-between gap-2 w-full lg:w-1/2 text-left">
              <div className="relative w-full flex items-center gap-2 text-sm translate-x-0 !ease-&lsqb;.24,0,.22,.99&rsqb; duration-200 group-hover:-translate-x-6 transition-transform">
                <Edit className="w-4 min-w-4 group-hover:opacity-0 transition-opacity" />
                <span>Blog post</span>
                {/* Removed: <ArrowRightIcon /> (missing icon) */}
              </div>
