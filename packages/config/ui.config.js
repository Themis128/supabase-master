...
import * as radixUiColors from '@radix-ui/colors'
import brandColors from './default-colors.js'
...
function radixColorKeys() {
  if (!radixUiColors || typeof radixUiColors !== 'object') return [];
  let keys = Object.keys(radixUiColors);

  // Filter out keys that are not color objects (e.g., __esModule, default)
  keys = keys.filter((key) => {
    const val = radixUiColors[key];
    return val && typeof val === 'object' && !Array.isArray(val);
  });

  // Remove dark variants and excluded colors
  keys = keys.filter(
    (key) =>
